...
from .fused_int4_linear import FusedInt4Linear, TRITON_AVAILABLE
...
def _patched_from_key(cls, key):
    ...
    if _PENDING_SWAPS:
        ...
        for name, info in _PENDING_SWAPS.items():
            ...
            if _should_eager_dequant(name):
                ...
                eager_count += 1
            elif _opts.use_fused and TRITON_AVAILABLE and _can_use_fused(
    ...
    if _opts.codec_int4:
        ...
        codec = _call_with_supported_kwargs(
            DACVAECodec.load,
            ...
            enable_watermark=bool(getattr(key, "enable_watermark", True)),
        )
